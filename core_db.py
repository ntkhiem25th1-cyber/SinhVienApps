...
def _resolve_page(page=None):
    ...
    try:
        while frame:
            locs = frame.f_locals
            # 1. Tìm biến page trực tiếp trong phạm vi hàm
            if "page" in locs and hasattr(locs["page"], "client_storage"):
                return locs["page"]
            # 2. Tìm qua tham số sự kiện e của Flet (on_click, on_change)
            if "e" in locs:
                e_val = locs["e"]
                if hasattr(e_val, "page") and hasattr(e_val.page, "client_storage"):
                    return e_val.page
                if hasattr(e_val, "control") and hasattr(e_val.control, "page") and hasattr(e_val.control.page,
                                                                                            "client_storage"):
